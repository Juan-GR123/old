...
if ($_SERVER["REQUEST_METHOD"] == "POST") {//hace que no salten los warning al entrar
    // Validación del nombre
    if (empty($_POST["name"])) {
        $nameErr = "El nombre es obligatorio.";
    } else {
        $name = test_input($_POST["name"]);//test_input= Función para limpiar y procesar datos
    ...
    if (empty($_POST["email"])) {
        ...
        if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {//filter_var: filtra una variable con un filtro especificado
...
function test_input($data) {
    $data = trim($data);//te quita los espacios en blanco
...
<body>  
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <p>Empleo actual:</p>
        <input type="radio" id="sin_empleo" name="employment" value="sin empleo" <?php if ($employment === "sin empleo") echo "checked"; ?>><!--El if sirve para despues de mandar el formulario mantenga marcados los valores que has enviado-->
