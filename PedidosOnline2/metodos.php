...
<body>
    <?php
    if ($_SERVER["REQUEST_METHOD"] == "POST") { //lo validamos
        ...
        function rellenaUsuarios()
        {
            ...
            function login($dni, $pw)
            {
                global $usuarios;
                if (empty($pw)) {
                    throw new Exception("<br>ERROR DEL SISTEMA: La contraseña no puede estar vacia");
                } elseif (!comprobarDNI($dni)) {
                    throw new Exception("<br>ERROR DEL SISTEMA: El dni es incorrecto");
                } else {
